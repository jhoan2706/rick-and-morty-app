...
class CharacterList
{
    public static function render(array $characters, ?int $selectedId = null, array $starredIds = []): string
    {
        ...
        $html .= '<h2 class="px-10 text-xs font-semibold uppercase tracking-wider text-gray-500 flex-shrink-0 ' . (empty($starred) ? '' : 'mt-8') . '">Characters (<span id="characters-count">' . count($regular) . '</span>)</h2>';
